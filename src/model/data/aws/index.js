//const MemoryDB = require('./memory-db'); -> Comment it in the lab9
...
async function listFragments(ownerId, expand = false) {
  ...
  // Limit to only `id` if we aren't supposed to expand. Without doing this we'll get back every attribute.
  // The projection expression defines a list of attributes to return, see:
...
async function deleteFragment(ownerId, id) {
  ...
  const dynamoDBCommand = new DeleteObjectCommand(dynamoDBParams);
  try {
    await ddbDocClient.send(dynamoDBCommand);
  } catch (err) {
    const { TableName, Key } = dynamoDBParams;
    logger.error({ err, TableName, Key }, 'Error delete fragment metadata from DynamoDB');
